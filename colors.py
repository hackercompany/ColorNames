...
def hslcode(color):
	rgb =[int(color[1:3],16) / 255 , int(color[3:5],16) / 255 , int(color[5:7],16) / 255]
	r = rgb[0]
	g = rgb[1]
	b = rgb[2]
	mi = min(r , min(g , b))
	...
	if delta > 0:
		...
		if ma == b and ma != r:
			h += (4 + (r - g) / delta)
		h /= 6
	return[int(h * 255) , int(s * 16) , int(l * 16)]
...
for i in range(len(names)):
	...
	r = rgbcode(color)
	h = hslcode(color)
	names[i].extend([r[0], r[1], r[2], h[0], h[1], h[0]])
